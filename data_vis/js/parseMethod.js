...
for (let i = 0; i < links.length; i++) {
    ...
    if(source.length != 1) {
        ...
        nodes.links[i].source = fName + "=>" + cName + "=>" + mName;
    } else {
        nodes.links[i].source = source[0];
    }
    if(target.length != 1) {
        mName = target[0];
        ...
        nodes.links[i].target = fName + "=>" + cName + "=>" + mName;
    }
}
...
for(let i = 0; i < links.nodes.length; i++) {
    ...
    if(id.length != 1) {
        ...
        sourceName = methodName + "=>" + className + "=>" + moduleName;
        for (let j = 0; j < nodes.nodes.length; j++) {
            if(sourceName == nodes.nodes[j].name) {
...
fs.writeFileSync("./d3withmock/data/methodGraph.json",JSON.stringify(nodes));